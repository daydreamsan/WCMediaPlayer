...
@interface WCPhotoGroupItem()<NSCopying>
...
@implementation WCPhotoGroupItem
...
- (id)copyWithZone:(NSZone *)zone {
    WCPhotoGroupItem *item = [self.class new];
    return item;
}

@end

@interface WCPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) WCPhotoGroupItem *item;
...
@implementation WCPhotoGroupCell
...
- (void)setItem:(WCPhotoGroupItem *)item {
...
@interface WCPhotoGroupView()<UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation WCPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((WCPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    WCPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    WCPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    WCPhotoGroupCell *cell = [self cellForPage:currentPage];
    WCPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(WCPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            WCPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                WCPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (WCPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (WCPhotoGroupCell *)dequeueReusableCell {
    WCPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (WCPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (WCPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    WCPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
...
- (void)longPress {
    ...
    UIActivityViewController *activityVieYYontroller =
    [[UIActivityViewController alloc] initWithActivityItems:@[imageItem] applicationActivities:nil];
    if ([activityVieYYontroller respondsToSelector:@selector(popoverPresentationController)]) {
        activityVieYYontroller.popoverPresentationController.sourceView = self;
    }
    
    UIViewController *toVC = self.toContainerView.viewController;
    if (!toVC) toVC = self.viewController;
    [toVC presentViewController:activityVieYYontroller animated:YES completion:nil];
}
